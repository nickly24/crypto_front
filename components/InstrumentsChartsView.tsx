  ...
  return (
    <div className="flex h-[calc(100vh-200px)] min-h-[500px] w-full">
      <aside className="w-44 shrink-0 flex flex-col gap-1 border-r border-[var(--card-border)] pr-3 py-1 overflow-y-auto">
        <p className="text-xs font-medium text-[var(--muted)] mb-2">Выберите для графика</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-2 min-h-0 overflow-hidden">
          <div className="flex-1 min-h-0 flex flex-col">
            <p className="text-xs text-[var(--muted)] shrink-0">{selected1 ? selected1.replace("-USDT-SWAP", "") : "Слот 1"}</p>
            <div className="flex-1 min-h-[180px]">
              <InstrumentChart instId={selected1} bar={bar} chartType={chartType} />
            </div>
          </div>
          <div className="flex-1 min-h-0 flex flex-col">
            <p className="text-xs text-[var(--muted)] shrink-0">{selected2 ? selected2.replace("-USDT-SWAP", "") : "Слот 2"}</p>
            <div className="flex-1 min-h-[180px]">
