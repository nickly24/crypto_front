...
import { HelpCircle, LogIn, LayoutDashboard, Settings, BarChart3 } from "lucide-react";
...
const publicNav = [
  ...
  { href: "/guide", label: "Гайд", icon: null },
  { href: "/faq", label: "Частые вопросы", icon: null },
];
  ...
  return (
    <header className="h-16 flex items-center justify-between px-6 lg:px-10 border-b border-white/5 bg-[var(--card-bg)]/60 backdrop-blur-xl sticky top-0 z-50">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${
                  active ? "text-[var(--accent)] bg-[var(--accent)]/10" : "text-[var(--muted)] hover:text-white"
                }`}
              >
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/guide"
          className="p-2 rounded-lg text-[var(--muted)] hover:text-white hover:bg-white/5 transition"
          title="Гайд"
        >
        ...
        {user ? (
          ...
          <Link
            href="/login"
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium hover:opacity-90 transition"
          >
